...
<div class="mycenterdeals-wrapper mycenterevent">
	...
	<?php else: ?>
			...
			<div class="mcd-event-cols">
				<div class="mcd-event-image-col">
					<div class="mcd-event-image">
						<img src="<?= $mycenterevent['event_image'] ?>" />
					</div>
				</div>
				...
				<div class="mcd-event-details">
					...
					<div class="mcd-event-date-time">
						...
						<?php if( !empty($mycenterevent['start_time']) && !$mycenterevent['all_day_event'] ) : ?>
					...
					<div class="mcd-event-add-to-calendar">
						<div title="Add to Calendar" class="addeventatc">
							...
							<span class="location"><?= $mycenterevent['center_name'].', '.$mycenterevent['center_location'] ?></span>
						</div>
					</div>
					<?php endif; ?>
					...
					<div class="mcd-event-share clearfix">
						<ul class="mcd-social-icons">
							...
							<!-- <li class="pinterest"><a href="http://pinterest.com/pin/create/button/?url=<?= get_current_url() ?>&media=<?= $mycenterevent['event_image'] ?>&description=<?= $mycenterevent['title'] ?>" target="_blank">Pinterest</a></li> -->
							<li class="email"><a href="mailto:?subject=<?= $mycenterevent['title'] ?>&body=Hi,%0D%0A%0D%0AEvent Details - <?= urlencode(get_current_url()) ?>%0D%0A%0D%0A<?= $mycenterevent['title'] ?>%0D%0A%0D%0A<?= urlencode($mycenterevent['event_description']) ?>%0D%0A%0D%0A<?= $mycenterevent['start_date'] ?> - <?= $mycenterevent['end_date'] ?>%0D%0A<?= $mycenterevent['start_time'] ?> - <?= $mycenterevent['end_time'] ?>%0D%0A%0D%0ACenter Location: <?= $mycenterevent['center_name'] ?>, <?= $mycenterevent['center_location'] ?>%0D%0A%0D%0A">Email</a></li>
						</ul>
					</div>
					<?php endif; ?>
				</div>
		...
		jQuery(document).ready(function($) {
			ga_event_tracking('EventsSingle', $('.mcd-event-name').text());
		});
		</script>
