...
<div ng-app="MyCenterPortalApp" ng-controller="BlogPostsCtrl" data-url="<?= MCD_API_BLOG_POSTS.'?center='.$mcd_center_id ?>">
	<div class="mycenterdeals-wrapper mycenterblog">
		...
		<div id="mycenterdeals-wrapper" ng-hide="data.error" ng-class="{loading: busy}">
			...
			<div id="mycenterblog" class="<?= ($isotope_grid?'isotope':'') ?>">
				<div class="mcd-blog-post-item" ng-class="{{post.categories | categoryClassesFromArr}}" ng-repeat="post in data.posts" ng-cloak>
					<a class="" href="<?= mcd_single_page_url('mycenterblogpost') ?>{{ post.slug }}">
						...
						<span class="mcd-blog-post-details">
							<span class="mcd-blog-post-title">{{ post.post_title }}</span>
